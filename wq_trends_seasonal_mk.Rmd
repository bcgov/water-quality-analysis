...
Inital results of seasonal Mann-Kendall trend tests of selected parameters at 
water quality monitoring stations from the Federal-Provincial water quality 
monitoring network.
...
for (s in station_results$SITE_NO) {
  ...
  site_name <- tools::toTitleCase(tolower(
    bc_stations[bc_stations$SITE_NO == s, "SITE_NAME"]
  ))
...
reporting_table <- left_join(
  bc_site_var_summary %>% 
    ...
    select(-nyears, -n_obs, -n_cens), 
  bc_nested %>%
    select(SITE_NO, PARAM_SHORT_NAME, annual_slope, slope_p_val), 
  by = c("SITE_NO", "PARAM_SHORT_NAME")
) %>% 
  mutate(PARAM_SHORT_NAME = tools::toTitleCase(tolower(PARAM_SHORT_NAME)), 
...
imap(split_tbl, ~ {
  cat("### ")
  .x %>% select(-Site) %>% 
  kable(booktabs = TRUE, longtable = TRUE, caption = .y) %>% 
