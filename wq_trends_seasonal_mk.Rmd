...
bc_nested <- mutate(
  bc_nested, 
  sk = map(full_monthly, ~ {
    EnvStats::kendallSeasonalTrendTest(.x$month_avg, season = .x$month, 
                                       year = .x$year)
  }), 
...
### Summary of data for each parameter at all sites:

```{r}
knitr::kable(bc_site_var_summary, booktabs = TRUE)
```
