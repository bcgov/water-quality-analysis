...
library(forcats)
library(lubridate)
library(wqbc) # install_github("bcgov/wqbc", ref = "develop")
library(readr)
library(here)
library(feather)
library(EnvStats)
...
```{r load, eval=FALSE || !file.exists(here::here("data/bcdata.rds"))}
...
bc_tidy <- tidy_ec_data(bc_data, cols = "PEARSEDA", mdl_action = "mdl") %>% 
  ...
  group_by(SITE_NO, PARAM_SHORT_NAME, Date, Units, DetectionLimit, 
  ...
  mutate(PARAM_SHORT_NAME = fct_reorder(PARAM_SHORT_NAME, as.numeric(as.factor(Category))),
         year = year(Date), 
         month = month(Date), 
         season = as.integer(cut(month, breaks = 4)),
...
bc_site_var_summary <- bc_tidy %>% group_by(SITE_NO, PARAM_SHORT_NAME, Category) %>% 
  summarize(min_year = min(year, na.rm = TRUE), 
            ...
            percent_censored = round((n_cens / n_obs) * 100, 1)) %>% 
  filter(max_year >= 2013, nyears >= 10) %>% 
  ungroup()

bc_nested <- inner_join(bc_tidy, bc_site_var_summary, 
                        by = c("SITE_NO", "PARAM_SHORT_NAME", "Category")) %>% 
  tidyr::nest(-PEARSEDA, -SITE_NO, -PARAM_SHORT_NAME, 
              -Category, -min_year, -max_year, -nyears, -n_obs, 
              -n_cens, -percent_censored)

# TODO: trim ends of ts when very sparse data
```

A total of `r nrow(wq_params)` parameters were selected to be tested. `r length(unique(bc_site_var_summary$SITE_NO))` of `r nrow(bc_stations)` monitoring 
stations had long enough (> 10 years) and recent enough (up to at least 2013) 
...
bc_nested <- bc_nested %>% 
  # don't include site/parameters with > 50% censored
  filter(percent_censored < 50) %>% 
  mutate(
  clean_data = map(data, ~ {
    # remove outliers > 6 sds from seasonal mean
    clean_wqdata(.x, sds = 6, by = "season", outlier_method = "sd",
...
bc_nested <- mutate(
  ...
  ## Convert to average annual percent change: 
  ## 100 * (Yend/Ystart - 1) / n_years.
...
saveRDS(bc_nested, file = here::here("tmp/bc_nested_results.rds"))
```
...
- Extreme outliers (> 6 Standard Deviations from the seasonal mean) were removed.
- Site/parameter combinations with > 50% censored data were removed.
...
for (s in station_results$SITE_NO) {
  d <- bc_nested %>% filter(SITE_NO == s) %>% 
    ...
    unnest() %>% 
    mutate(censored = case_when(censored != "none" ~ "censored", 
                                TRUE ~ "not censored"))
  ...
  p <- ggplot(d, aes(x = Date, y = month_avg)) + 
    ...
    geom_point(aes(alpha = censored), colour = "grey30", size = 0.4) +
    scale_alpha_manual(values = c("censored" = 0.4, "not censored" = 0.8)) + 
    ...
    scale_colour_manual(values = c("Increasing" = "darkorange2", 
                                   "Decreasing" = "dodgerblue2")) + 
