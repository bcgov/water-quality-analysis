...
for (i in seq_along(split_tbl)) {
  ...
  .x %>% 
    select(PARAM_SHORT_NAME, Category, Years, n_obs, percent_censored, 
           ann_percent_change, slope_p_val, n_outliers) %>% 
    kable(booktabs = TRUE, longtable = TRUE, 
          caption = paste0(site_name, " (", .y, ")"), 
          col.names = c("Parameter", "Category", "Years", "N obs", 
                        "\\% cens", "Ann \\% $\\Delta$", "p val", "outliers"), 
          escape = FALSE) %>% # Use esacape=F and manually escape special characters to make greek letter Delta work
    kable_styling(latex_options = c("striped", "repeat_header")) %>% 
    print()
}
