...
This presents draft initial results of seasonal Mann-Kendall trend tests of 
...
bc_tidy <- tidy_ec_data(bc_data, cols = "PEARSEDA", mdl_action = "mdl") %>% 
  add_shortnames() %>% 
  filter(PARAM_SHORT_NAME %in% wq_params$Variable_short_name, 
         year(DateTime) > 2003) %>% 
...
kable(wq_params, booktabs = TRUE, longtable = TRUE, 
      caption = "List of water quality parameters tested for trends") %>% 
...
statistically significant trends. The parameter that showed sigicant decreases at the most sites
was `r names(which.max(table(bc_nested[bc_nested$sig & bc_nested$annual_slope < 0, "PARAM_SHORT_NAME"])))` (`r max(table(bc_nested[bc_nested$sig & bc_nested$annual_slope < 0, "PARAM_SHORT_NAME"]))` sites). The parameter that showed sigicant increases at the most sites
...
- Data from 2004 and later were used (due to changes in detection limits/instrumentation - this can be changed and warrants further discussion)
...
- Records were not adjusted for river flow/levels. This decision was made so that we are reporting on real concentrations of water quality parameters, not on why they might be changing. This is in keeping with SoE's mandate of reporting on _what_ is happening, not necessarilly delving into the _why_.

### Data Preparation issues to address:

- Check parameters with very high within-day variation - especially `TEMPERATURE WATER` vs `TEMPERATURE WATER (FIELD)`
- Trim ends of time series when they are very sparse (E.g., Sulphate at 'Englishman River at Hwy 19').
- Better deal with censored data?
- Look at what earlier data (earlier than 2004) should be used

## Trend analysis
- For each parameter at each station, we performed a Seasonall Kendall test with Sen slope estimator using the `seaken` function in the [`smwrStats` R package](https://github.com/USGS-R/smwrStats). This performs a separate Mann Kendall for each month, and reports the median of all monthly Sen's slopes. The significance of the Seasonal Kendall is assessed using the overall Sk statistic, which is calculated by summing each season's Kendall S statistic.

```{r results='asis'}
station_results <- bc_nested %>%
  select(SITE_NO, PARAM_SHORT_NAME, sk, slope_p_val, sig, annual_slope, intercept) %>% 
...
for (s in station_results$SITE_NO) {
  ...
  res <- station_results %>% 
    ...
    mutate(slope_disp = ifelse(sig, 
                               format(annual_slope, trim = TRUE, digits = 1, 
                                      scientific = 1, drop0trailing = TRUE), 
                               NA_character_))
  ...
  p <- ggplot(d, aes(x = Date, y = month_avg)) + 
    ...
    geom_label(data = res, x = max(d$Date), 
    ...
    labs(title = site_name, 
         ...
         caption = "Slope of significant trends (in units/year) are shown on each graph panel",
         x = "Date", y = "Value", 
         colour = "Significant Trend", 
         alpha = "Censored")
  print(p)
  # Add newlines between every second chart
  if (i %% 2 != 0) {
    cat("\\newline")
    cat("\\newline")
  }
...
reporting_table <- left_join(
  bc_site_var_summary %>% 
    ...
    select(-n_cens), 
  bc_nested %>%
    select(SITE_NO, PARAM_SHORT_NAME, annual_slope, slope_p_val), 
  by = c("SITE_NO", "PARAM_SHORT_NAME")
) %>% 
  mutate(PARAM_SHORT_NAME = to_titlecase(PARAM_SHORT_NAME), 
         annual_slope = round(annual_slope, 3), 
         slope_p_val = round(slope_p_val, 3)) %>% 
  rename(Site = SITE_NO, Parameter = PARAM_SHORT_NAME, 
         `Slope (Units/yr)` = annual_slope, p = slope_p_val)
