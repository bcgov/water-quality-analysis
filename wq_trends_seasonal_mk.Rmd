...
- Data from 2004 on were retained (due to changes in detection limits - this can be changed)
- Multiple readings from the same day were averaged (this was rare; only `r sum(bc_tidy$n > 1)` records had more than one daily reading))
...
- Check params with very high dailiy CV - especially `TEMPERATURE WATER` vs `TEMPERATURE WATER (FIELD)`
...
i <- 1
for (s in station_results$SITE_NO[1:5]) {
  ...
  p <- ggplot(d, aes(x = Date, y = month_avg)) + 
    ...
    geom_text(data = res, x = max(d$Date), 
