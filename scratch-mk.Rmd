...
bc_nested %>% filter(SITE_NO == s) %>% 
  select(SITE_NO, PARAM_SHORT_NAME, full_monthly) %>% 
  unnest() %>% 
  ggplot(aes(x = .data$Date, y = .data$month_avg)) + 
  geom_point() + 
  geom_abline(data = dummy, aes(slope = .data$slope / 365, 
                                intercept = .data$intercept -
    ((.data$slope / 365) * min(as.numeric(as.Date("2004-01-15")))), 
    colour = .data$sig)) +
  # geom_text(data = dummy, aes())
  facet_wrap(vars(PARAM_SHORT_NAME), scales = "free_y") + 
  labs(x = "Date", y = "Value", colour = "Significance", 
       title = s)
...
Test usgs `restrend` Package
```{r}
# install.packages("restrend", repos = 'https://owi.usgs.gov/R')
library(restrend)
library(smwrQW)

test_data <- filter(bc_nested, SITE_NO == "BC08FC0001", PARAM_SHORT_NAME == "ZINC TOTAL") %>% 
  select(clean_data) %>% 
  unnest()

test_cens <- as.lcens(test_data$Value, test_data$DetectionLimit)
