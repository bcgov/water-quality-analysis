...
# bc_nested %>% select(SITE_NO, PARAM_SHORT_NAME, full_monthly) %>% unnest %>% View()
...
library(astsa)
mag_ts <- as.ts(bc08fc0001_mag$month_avg)
plot(mag_ts)
acf2(mag_ts, max.lag = 60)
dl_mag_ts <- diff(log(mag_ts))
plot(dl_mag_ts)
acf2(dl_mag_ts)
sarima(dl_mag_ts, p = 0, d = 0, q = 0)
...
ggplot(bc08fc0001_mag, aes(x = time, y = month_avg)) + 
  geom_point() + 
  geom_ribbon(data = pred_data, aes(ymin = lower_sim_ci, ymax = upper_sim_ci, x = time, y = fitted),
                 alpha = 0.8, fill = "grey") +
    geom_line(data = pred_data, aes(y = exp(fitted), x = time))
