...
bc08fc0001_mag <- filter(bc_nested, SITE_NO == "BC08FC0001", 
...
ggplot(newd, aes(x = Time, y = derivative)) +
...
ggplot(pred_data, aes(x = Date, y = pred)) +
  ...
  geom_line() + 
  geom_line(data = pred_data[!is.na(newd$increasing) | !is.na(newd$decreasing), ], 
            size = 2, colour = "blue")
