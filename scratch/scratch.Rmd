...
pred_data <- mutate(pred_data,
                    fderiv = fd_ci$est, # computed first derivative
                    fd_upper = fd_ci$upper,  # upper CI on first deriv
                    fd_lower = fd_ci$lower,  # lower CI on first deriv
                    # where is curve increasing?
                    increasing = ifelse(fd_ci$incr, fderiv, NA_real_),
                    # where is curve decreasing?
                    decreasing = ifelse(fd_ci$decr, fderiv, NA_real_),
                    direction = ifelse(fd_ci$incr, "increasing", 
                                       ifelse(fd_ci$decr, "decreasing", 
                                              NA)))
